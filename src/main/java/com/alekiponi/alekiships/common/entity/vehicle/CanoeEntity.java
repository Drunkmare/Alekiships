...
public class CanoeEntity extends AbstractAlekiBoatEntity {
    ...
    public InteractionResult interact(final Player player, final InteractionHand hand) {
        ...
        if (stack.is(AlekiShipsItems.CANOE_PADDLE.get()) && player.isCreative() && player.isSecondaryUseActive()) {
            if(this.getLength() == 5){
                this.setLength(3);
            } else {
    ...
    protected float getPaddleMultiplier() {
        ...
        if(this.getControllingPassenger() instanceof Player player){
            if (player.isHolding(AlekiShipsItems.CANOE_PADDLE.get())) {
        ...
        for (Entity entity : this.getTruePassengers()) {
            if (entity instanceof Player player2) {
                if (player2.isHolding(AlekiShipsItems.CANOE_PADDLE.get())) {
                    i++;
                }
            }
        }
