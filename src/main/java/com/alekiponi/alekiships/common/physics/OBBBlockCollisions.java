...
public class OBBBlockCollisions<T> extends AbstractIterator<T> {
    ...
    public OBBBlockCollisions(CollisionGetter pCollisionGetter, @Nullable Entity pEntity, AABB pBox, OBB pObb, boolean pOnlySuffocatingBlocks, BiFunction<BlockPos.MutableBlockPos, VoxelShape, T> pResultProvider) {
        ...
        this.obb = pObb;
        if (pBox != pObb.getExtent()) {
            this.aabb = pObb.getExtent();
        } else {
            this.aabb = pBox;
        }
        ...
        while (true) {
            if (this.cursor.advance()) {
                ...
                if (voxelshape == Shapes.block()) {
                    if (!this.obb.intersects((double) i, (double) j, (double) k, (double) i + 1.0D, (double) j + 1.0D, (double) k + 1.0D)) {
                        continue;
                    }
